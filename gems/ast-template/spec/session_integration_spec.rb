...
RSpec.describe Ast::Template do
  ...
  it "conforms to the template directory session status transport envelope fixture" do
    ...
    fixture.fetch(:cases).each do |test_case|
      ...
      expect(json_ready(described_class.template_directory_session_status_envelope(input))).to eq(json_ready(expected))
      expect(described_class.import_template_directory_session_status_envelope(expected)).to eq([input, nil])
    end
  end
  ...
  it "conforms to the template directory session status envelope application fixture" do
    ...
    fixture.fetch(:cases).each do |test_case|
      envelope = test_case.fetch(:envelope)
      status, error = described_class.import_template_directory_session_status_envelope(envelope)

      expect(error).to be_nil
      expect(json_ready(status)).to eq(json_ready(test_case.fetch(:expected)))
    end
  ...
  it "conforms to the template directory session diagnostics transport envelope fixture" do
    ...
    fixture.fetch(:cases).each do |test_case|
      ...
      expect(json_ready(described_class.template_directory_session_diagnostics_envelope(input))).to eq(json_ready(expected))
      expect(described_class.import_template_directory_session_diagnostics_envelope(expected)).to eq([input, nil])
    end
  end
