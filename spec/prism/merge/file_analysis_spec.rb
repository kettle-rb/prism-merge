...
RSpec.describe Prism::Merge::FileAnalysis do
  ...
  describe "#freeze_blocks" do
    it "extracts freeze block information" do
      ...
      expect(freeze_block[:text]).to include("# Custom content")
      expect(freeze_block[:text]).to include('gem "custom"')
    end
  ...
  describe "#nodes_with_comments" do
    it "extracts nodes with their associated comments" do
      ...
      node_info = nodes.first
      expect(node_info[:node]).to be_a(Prism::DefNode)
      expect(node_info[:leading_comments].length).to eq(1)
      expect(node_info[:inline_comments].length).to eq(1)
    end
  end
end
